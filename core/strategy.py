"""
BB Squeeze Breakout Strategy — v2.0 (4H Edition)
=================================================
Upgrades from backtest analysis:
  - Primary TF: 4H (42% WR vs 34% on 30m)
  - Confirmation: 1H trend alignment
  - Partial TP at 1R (secure quick profit)
  - Trailing SL (ride the trend)
  - Vol filter 1.2x (4H-calibrated)
  - Min squeeze 2 bars (4H appropriate)
"""
import numpy as np
import pandas as pd
...
from config.settings import (
    ...
    PARTIAL_TP_R, PARTIAL_TP_SIZE, USE_PARTIAL_TP,
    TRAIL_ACTIVATE_R, TRAIL_DISTANCE_ATR, USE_TRAILING_SL,
)
...
class Signal:
    ...
    entry:         float
    sl:            float
    tp:            float        # Full TP (2.5R)
    tp1:           float        # Partial TP (1R) — new
    atr:           float
    confidence:    float
    squeeze_dur:   int
    breakout_str:  float
    ...
    timestamp:     str
    reason:        str
    # Trailing SL tracking — updated during trade monitoring
...
class StrategyEngine:
    ...
    def _compute_supertrend(self, df, period=None, mult=None):
        ...
        upper = hl2 + mult * atr
        lower = hl2 - mult * atr
        st = [lower.iloc[0] if not pd.isna(lower.iloc[0]) else 0]
        for i in range(1, len(df)):
            c  = df['close'].iloc[i]
            li = lower.iloc[i] if not pd.isna(lower.iloc[i]) else st[-1]
            ui = upper.iloc[i] if not pd.isna(upper.iloc[i]) else st[-1]
            st.append(max(li, st[-1]) if c > st[-1] else min(ui, st[-1]))
        return df['close'] > pd.Series(st, index=df.index)
    ...
    def prepare_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 4H Trend (resampled from whatever TF is primary)
    ...
    def check_signal(self, symbol: str, df: pd.DataFrame) -> Optional[Signal]:
        """
        4H BB Squeeze signal detection.
        Returns Signal with tp1 (partial) and tp (full) levels.
        """
        if len(df) < 50:  # 4H needs fewer bars
            return None
        ...
        # CONDITION 2: Min squeeze duration (2 bars on 4H)
        ...
        # CONDITION 6: Volume filter (1.2x for 4H)
        vol_ratio = float(row.get('vol_ratio', 1.0))
        if pd.isna(vol_ratio) or vol_ratio < BB_VOL_FILTER:
            return None

        # CONDITION 7: Supertrend
        ...
        if direction == 'long':
            ...
            tp1 = entry + abs(entry - sl) * PARTIAL_TP_R   # 1R
            tp  = entry + abs(entry - sl) * RR_RATIO       # 2.5R
        else:
            sl  = entry + atr * sl_mult
            tp1 = entry - abs(sl - entry) * PARTIAL_TP_R   # 1R
            tp  = entry - abs(sl - entry) * RR_RATIO       # 2.5R

        # Confidence score
        score = 0.5
        if vol_ratio > 1.5:  score += 0.08
        if vol_ratio > 2.0:  score += 0.07
        sq_dur = int(row.get('squeeze_dur', 0))
        if sq_dur > 3:       score += 0.08
        if sq_dur > 6:       score += 0.05
        ...
        return Signal(
            ...
            reason       = reason,
            trail_sl     = sl,   # starts at original SL
        )

    def update_trailing_sl(self, trade: dict, current_price: float) -> dict:
        """
        Update trailing SL for an open trade.
        Returns dict with: new_sl, trail_active, partial_done, should_close_partial
        """
        entry     = float(trade['entry_price'])
        original_sl = float(trade['sl_price'])
        atr       = float(trade.get('atr', abs(entry - original_sl) / 0.8))
        direction = trade['direction']
        risk      = abs(entry - original_sl)
        tp1       = float(trade.get('tp1_price', entry + risk * PARTIAL_TP_R if direction=='long'
                                    else entry - risk * PARTIAL_TP_R))

        current_sl       = float(trade.get('trail_sl', original_sl) or original_sl)
        trail_active     = bool(trade.get('trail_active', False))
        partial_done     = bool(trade.get('partial_done', False))
        should_close_partial = False

        if direction == 'long':
            profit_r = (current_price - entry) / max(risk, 0.001)

            # Check partial TP1
            if USE_PARTIAL_TP and not partial_done and current_price >= tp1:
                should_close_partial = True

            # Activate trailing SL
            if USE_TRAILING_SL and profit_r >= TRAIL_ACTIVATE_R:
                trail_active = True
                # Trail SL = current price - (ATR * trail_distance)
                new_trail = current_price - (atr * TRAIL_DISTANCE_ATR)
                # SL can only move UP (never down)
                # Minimum: breakeven (entry)
                new_trail = max(new_trail, entry)
                new_trail = max(new_trail, current_sl)
                current_sl = round(new_trail, 4)

        else:  # short
            profit_r = (entry - current_price) / max(risk, 0.001)
            ...
            if USE_TRAILING_SL and profit_r >= TRAIL_ACTIVATE_R:
                ...
                new_trail = min(new_trail, current_sl)
                current_sl = round(new_trail, 4)
        ...
        return {
            ...
            'should_close_partial': should_close_partial,
